...
import brand from "@/../public/images/brand5.jpg";
...
export default function CardAddToCart({ brandId, off, price, isLiked }: CardAddToCartProps) {
	return (
		<div
			className='h-fit min-w-fit flex items-center flex-row-reverse lg:flex-col justify-between lg:justify-normal z-30 border-2 bg-neutral-100 rounded-lg shadow-inner lg:divide-y-2 divide-neutral-200 px-3 py-5 overflow-hidden
		 '>
			<div className='brand-wrapper flex-col gap-2 items-center h-40 w-full hidden lg:flex '>
				...
				<span className='text-neutral-700 hover:text-neutral-900 text-xs'>
					<Link href={"/"}>برای مشاهده همه محسولات برند یو</Link>
				</span>
			</div>
				...
				<div className='flex flex-col sm:flex-row items-center gap-4'>
					...
					<div className='flex flex-col gap-2'>
						{price === 0 ? (
							...
							<>
								{price > 0 && !off && (
									<span className='text-sm sm:text-base xl:text-lg font-bold'>{price}</span>
								)}
								{price > 0 && off && (
								...
								{off && off > 0 && (
									<span className='text-sm sm:text-base xl:text-lg font-bold text-secondary-700'>
										{calculateDiscountedPrice(price, off).toLocaleString()} تومان
									</span>
								)}
							</>
						)}
