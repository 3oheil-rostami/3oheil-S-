...
const Register = () => {
		...
		toast.update(toastId, {
			...
			render() {
				...
				return response.status === 500
					...
					: response.status === 429
					? response.json().then(message => message)
					: response.status === 400
					? "کاربری با این شماره موجود نیست ."
					: response.status === 200
	...
	const otherFormOnSubmit = otherHandleSubmit(async ({ code, confirmPassword, password }) => {
		if (!navigator.onLine) {
			toast.warning("شما آنلاین نیستید .");
			return;
		}
		...
		toast.update(toastId, {
			...
			type: response.ok ? "success" : "error",
			render() {
				setTimeout(() => response.status === 200 && router.replace("/login"), 3000);
				return response.status === 500
					? "خطایی در سرور رخ داده است ."
					: response.status === 404
	...
	return (
		<LoginLayout>
			<div className="bg-white px-5 py-10 rounded-2xl max-w-96">
				<h1 className="text-gray-800 font-bold text-2xl mb-1">بازیابی رمز عبور</h1>
				...
				<p className="text-sm font-bold text-gray-600 mb-3 mr-2">
					اول شماره خود را وارد کنید و دکمه بغلش را بزنید تا کد براتون ارسال شود بعد میتوانید ادامه
					دهید.
				</p>
				<form className="" onSubmit={submitSendPhoneNumberHandler}>
					<LoginInputWrapper
						...
						<Button
							...
							variant="fill"
							size="2xs"
							style={{ color: "#d71d44", marginRight: "auto" }}>
							ارسال کد
						</Button>
					</LoginInputWrapper>
				</form>
				<form onSubmit={submitSendOtherFormHandler}>
					<LoginInputWrapper
						...
						<input
							className="pr-2 outline-none border-none font-sans"
							type="text"
							placeholder="کد ارسالی"
							{...otherRegister("code", {
								required: { value: true, message: "کد ارسالی را وارد نکردید !" },
								minLength: { value: 5, message: "کد وارد شده کمتر از 5 کاراکتر است." },
								maxLength: { value: 5, message: "کد وارد شده بیشتر از 5 کاراکتر است." },
								disabled: !isEnableOtherForm,
							})}
						/>
					</LoginInputWrapper>
					...
					<LoginInputWrapper
						...
						<input
							...
							{...otherRegister("confirmPassword", {
								...
								minLength: { value: 8, message: "تکرار رمز عبور نباید کمتر از 8 کاراکتر باشد ." },
								maxLength: {
									value: 25,
