...
const page = async ({ params: { productId } }: Props) => {
  ...
  const otherData = await getOtherData(data?.product._id || "");
        ...
        <div className="flex flex-col lg:flex-row justify-center gap-4">
          <div className="relative">
            <ImageSlider
              images={[
                {
                  src: `${process.env.NEXT_PUBLIC_DEVELOPMENT_ENV_BACKEND_URL}/images/productCover/${data?.product.productCover}`,
                  alt: "product image",
                },
                ...(data?.product.productPics.map((picItem) => ({
                  src: `${process.env.NEXT_PUBLIC_DEVELOPMENT_ENV_BACKEND_URL}/image/productPic/${picItem}`,
                  alt: `product image`,
                })) || []),
              ]}
            />
          ...
          <ProductImages
            images={[
              {
                src: `${process.env.NEXT_PUBLIC_DEVELOPMENT_ENV_BACKEND_URL}/image/productCover/${data?.product.productCover}`,
                alt: "product image",
              },
              ...(data?.product.productPics.map((picItem) => ({
                src: `${process.env.NEXT_PUBLIC_DEVELOPMENT_ENV_BACKEND_URL}/image/productPic/${picItem}`,
          ...
          <div className="grow">
            ...
            <div className="flex gap-10 flex-col lg:flex-row lg:gap-3">
              ...
              <CardAddToCart
                ...
                off={
                  data?.product.colors.sort((a, b) => a.off - b.off)[0].off || 0
                }
                isLiked={!!otherData?.checkLickedProductResponse}
              />
            </div>
        ...
        <section>
          ...
          <ul className="mr-3 pr-3 mt-2 border-r border-neutral-400 p-5 rounded-lg bg-neutral-100">
            {data?.product.info.map((infoItem, index) => (
              <li
                key={index}
                className="flex gap-3 py-2 text-base font-bold text-neutral-700"
              >
